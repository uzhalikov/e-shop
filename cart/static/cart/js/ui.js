export const createOrderForm = (formData) => {
    return `
    <form id="base_order" class="form unselect">
        <h3>Оформление заказа</h3>
        ${formData}
        <button type="submit">Оформить</button>
    </form>`
}
export const createQuickOrderForm = (formData) => {
    return `
    <form id="quick_order" class="form unselect">
        <h3>Оформление быстрого заказа</h3>
        ${formData}
        <button type="submit">Оформить</button>
    </form>`
}
...
export const selectTypeOrder = () => {
    return `
        <div style="display: flex; gap: 10px; padding: 20px;">
            <span class="simple-link" id="order_one_click">Купить в один клик</span>
            <span class="simple-link" id="enter">Авторизоваться</span>
        </div>`
}